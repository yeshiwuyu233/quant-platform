...
def _read_future_returns(all_dates, start_idx, codes):
    ...
    for day_i, fd in enumerate(future_dates):
        try:
            df_f = _prepare_df(pd.read_excel(MASTER_FILE, sheet_name=fd))
        except Exception:
            continue
        ret_map = {}
...
def _build_top_industry_weekly(all_pool, ind_counts, daily_map, n_available):
    ...
    for ind_name in selected:
        ...
        count = len(cum_vals)
        avg_ret = cum_vals.mean() if count > 0 else 0.0
        win_rate = (cum_vals > 0).mean() if count > 0 else 0.0
...
def generate_weekly_report(sheet_name, all_dates, current_idx):
    ...
    # ── 读取 T 日数据 ──
    try:
        df_T = pd.read_excel(MASTER_FILE, sheet_name=sheet_name)
    except Exception as e:
        log.error(f"  读取 [{sheet_name}] 失败: {e}")
        return False
    ...
    # ── 写 JSON ──
    json_path = os.path.join(PROJECT_ROOT, f"{sheet_name}的选股策略礼拜攻势.json")
...
def _build_weekly_trend_cache():
    ...
    records = []
    for f in sorted(glob.glob(os.path.join(PROJECT_ROOT, "*的选股策略礼拜攻势.json"))):
...
def run_incremental_weekly(date_str):
    ...
    all_dates = sorted(xls.sheet_names, key=lambda s: int(s))
...
def main():
    ...
    for i, d in enumerate(dates):
        json_path = os.path.join(PROJECT_ROOT, f'{d}的选股策略礼拜攻势.json')
