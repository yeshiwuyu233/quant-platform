...
def _build_pipeline_email(date_str, backtest_ok, weekly_ok, elapsed):
    ...
    # ── Backtest results ──
    bt_json = os.path.join(_PROJECT_ROOT, f"{date_str}量化复盘报告.json")
    ...
    try:
        ...
        all_sheets = sorted(xls.sheet_names, key=lambda s: int(s))
        bt_dates_set = set()
        for f in glob.glob(os.path.join(_PROJECT_ROOT, "*量化复盘报告.xlsx")):
        ...
        if date_str in ordered_dates:
            ...
            if settle_idx >= 0:
                ...
                wk_json = os.path.join(_PROJECT_ROOT, f"{settle_date}的选股策略礼拜攻势.json")
    ...
    wk_new_json = os.path.join(_PROJECT_ROOT, f"{date_str}的选股策略礼拜攻势.json")
...
def _run_pipeline_for_date(date_yyyymmdd, temp_csv=None):
    """Background thread: write to master → run backtest + weekly for a given date."""
    import subprocess
    import sys
    import pandas as pd
    from datetime import datetime
