...
def run_backtest(sheet_yesterday, sheet_today):
    ...
    df_yest = pd.read_excel(xls, sheet_name=sheet_yesterday)
    df_today = pd.read_excel(xls, sheet_name=sheet_today)

    df_yest = split_empty_industries(df_yest)
    df_today = split_empty_industries(df_today)
    df_yest = df_yest.dropna(subset=['代码', '今日指标'])
    df_today = df_today.dropna(subset=['代码', '指标历史'])
    ...
    for t in [0.8, 1.0, 1.2]:
        ...
        temp_merged = pd.merge(temp_pool, df_today_subset, on='代码', how='inner')
        total_n = len(temp_merged)
        success_n = (temp_merged['今日收益率'] >= 0).sum() if total_n > 0 else 0
        win_rate_rows.append({
            '策略条件': f'准确率>0.6 & 指标>{t}',
            '总入选股数': total_n,
            '成功数(收益>=0)': success_n,
            '失败数(收益<0)': total_n - success_n,
            '策略胜率': f"{(success_n/total_n):.2%}" if total_n > 0 else "0.00%"
        })
    ...
    backtest_df = pd.merge(target_pool, df_today_subset[['代码', '指标历史', '今日收益率']], on='代码', how='inner')
    backtest_df['回测结果'] = np.where(backtest_df['今日收益率'] >= 0, '成功', '失败')
    ...
    if not ind_stat.empty:
        real_ind = ind_stat[~ind_stat['行业名称'].astype(str).str.match(r'^空\d*$')]
    ...
    for t in [0.8, 1.0, 1.2]:
        ...
        col_map[t] = f"{row['策略胜率']}({row['成功数(收益>=0)']}/{row['总入选股数']})" if row and row['总入选股数'] > 0 else ""
    ...
    nc_tot = len(nc_df)
    nc_suc = (nc_df['今日收益率'] >= 0).sum() if nc_tot > 0 else 0
    nc_str = f"{nc_suc/nc_tot:.2%}({nc_suc}/{nc_tot})" if nc_tot > 0 else "0.00%(0/0)"

    t3_df = backtest_df[backtest_df['行业'].isin(top_3_inds)]
    t3_tot = len(t3_df)
    t3_suc = (t3_df['今日收益率'] >= 0).sum() if t3_tot > 0 else 0
    t3_str = f"{t3_suc/t3_tot:.2%}({t3_suc}/{t3_tot})" if t3_tot > 0 else "0.00%(0/0)"
...
def _rebuild_history():
    ...
    history = []
    for jf in sorted(glob.glob(os.path.join(PROJECT_ROOT, '*量化复盘报告.json'))):
        try:
...
def run_single_backtest(date_str):
    ...
    xls = pd.ExcelFile(MASTER_FILE)
    dates = sorted(xls.sheet_names, key=lambda s: int(s))
    ...
    fn = os.path.join(PROJECT_ROOT, f'{curr}量化复盘报告.xlsx')
...
def main():
    ...
    for f in glob.glob(os.path.join(PROJECT_ROOT, '*量化复盘报告*.xlsx')):
        try:
            df = pd.read_excel(f, sheet_name='0.每日追踪总表', header=[0, 1])
            row = df.dropna(how='all').iloc[0]
            existing_reports.add(f"{int(row.iloc[1]) % 10000:04d}")
        except Exception:
            pass
